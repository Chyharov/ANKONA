...
{post.content && (
  <p className={s.blogDescription}>
    {post.content}
    {post.contentspan && <span className={s.blogDescriptionSpan}>{post.contentspan}</span>}
...
{post.image && <img className={s.blogImage} src={post.image} alt="blogImage" />}
...
{post.afterResult && <p className={s.afterResult}>{post.afterResult}</p>}
...
{post.link && (
  <Link to={post.link} target="_blank" rel="noopener noreferrer" className={s.blogLinkTo}>
    Джерело
  </Link>
)}
...
{post.additationInfo && post.blogAddInfoLink && post.blogAddInfo && (
  <p className={s.additationInfo}>
    {post.additationInfo}
    <Link to={post.blogAddInfoLink} target="_blank" rel="noopener noreferrer" className={s.blogAddInfoLink}>
      {post.blogAddInfo}
    </Link>
  </p>
)}
